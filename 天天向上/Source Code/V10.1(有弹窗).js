...
if (jifen_list.child(2).child(3).text() != "已完成") { // 视听学习
  console.verbose("无障碍服务："+auto.service);
  toastLog("开始视听次数");
  do_shipin();
  jifen_list = refind_jifen();
}
if (jifen_list.child(1).child(3).text() != "已完成") { // 文章
  console.verbose("无障碍服务："+auto.service);
  toastLog("开始文章次数与时长");
	do_wenzhang();
  jifen_list = refind_jifen();
}
if (jifen_list.child(4).child(3).text() != "已完成") { // 每日答题
  toastLog("每日答题开始");
  do_meiri();
  jifen_list = refind_jifen();
}
var meizhou_right = 1;
if (jifen_list.child(5).child(2).text().match(/\d+/)[0] == "0") { // 每周答题
  toastLog("每周答题开始");
...
if (jifen_list.child(6).child(2).text().match(/\d+/)[0] == "0") { // 专项答题
  toastLog("专项答题开始");
  var zhuanxiang_right = do_zhuanxiang();
  jifen_list = refind_jifen();
}
if (jifen_list.child(7).child(3).text() != "已完成") { // 挑战答题
  toastLog("挑战答题开始");
  do_tiaozhan();
  jifen_list = refind_jifen();
}
...
if (test_result) {
  console.verbose("无障碍服务："+auto.service);
  jifen_list = refind_jifen();
  if (parseInt(jifen_list.child(8).child(2).text().match(/\d+/)[0]) <= 3) { // 四人两次
    toastLog("四人赛开始");
    do_duizhan1(4);
    do_duizhan1(4);
    jifen_list = refind_jifen();
  }
  if (jifen_list.child(9).child(2).text().match(/\d+/)[0] == "0") { // 双人对战
    toastLog("双人对战开始");
    do_duizhan1(2);
    jifen_list = refind_jifen();
  }
...
var dingyue_right = 1;
if (jifen_list.child(10).child(2).text().match(/\d+/)[0] == "0") { // 订阅
  toastLog("订阅开始");
