...
var url = 'https://gitee.com/sec-an/js/raw/master/QuestionBank.db';
var path = '/sdcard/QuestionBank.db';
device.wakeUpIfNeeded(); //点亮屏幕
var first = true; //记录答题的第一次
var r; // 替换用；
var meizhou_txt = helper_bot.checkbox_02;
var zhuanxiang_txt = helper_bot.checkbox_01;
var siren = helper_bot.checkbox_03;
var shuangren = helper_bot.shuangren;

var articles = helper_bot.article;
var video = helper_bot.video;
var meiri = helper_bot.meiri;
var tiaozhan = helper_bot.tiaozhan;

var choose = helper_bot.select_01;

var 专项答题下滑 = helper_bot.select;
var 每周答题下滑 = helper_bot.selectm;

var 订阅 = helper_bot.ssub;
// var 随机 = helper_bot.suiji;

var stronger = helper_bot.stronger; //每日答题增强模式
var token;
var ttt;
...
var init_url = 'https://gitee.com/sec-an/js/raw/master/question';
var file_tmp = false;
var tikus = '';
/**
 * 获取用户token
...
function get_baidu_token() { // 百度ocr
    if (!helper_bot.AK || !helper_bot.SK) {
    ...
    var res = http.post(
        'https://aip.baidubce.com/oauth/2.0/token', {
            ...
            client_id: helper_bot.AK.replace(/ /g, ''),
            client_secret: helper_bot.SK.replace(/ /g, '')
        }
    );
...
function show_log() {
    threads.start(function () {
        try {
            var text = http.get('https://gitee.com/sec-an/js/raw/master/showlogs').body.string();
            if (text.length == 0) {
                show_log = true;
                return;
            }
...
var aTime = helper_bot.time1; //有效阅读一分钟1分*6
var vTime = helper_bot.time2; //每个小视频学习-5秒
...
function init() {
    ...
    threads.start(function () {
        try {
            var x = http.get('https://gitee.com/sec-an/js/raw/master/replace.js').body.string();
...
function zsyAnswer() {
    ...
    if (helper_bot.another)
        var x = helper_bot.another * 1;
    else
        var x = 0;
    while (x > 0) {
...
function push_score() {
    ...
    score += '\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n';
    try {
        url = 'https://pushplus.hxtrip.com/send?token=' + helper_bot.Token.replace(/ /g, '') + '&title=Study&content=' + score + '&template=html';
        http.get(url);
    } catch (e) {}
    try {
        url = 'http://www.pushplus.plus/send?token=' + helper_bot.Token.replace(/ /g, '') + '&title=Study&content=' + score + '&template=html';
        http.get(url);
    } catch (e) {}
}
...
function rand_mode() {
    ...
    article_list = null;
    back_table();
    if (helper_bot.Token != null && helper_bot.Token.length > 6) {
        delay(1);
        push_score();
    }
...
function diandian() {
    if (helper_bot.diandian == null || helper_bot.diandian == false) return;
    while (!desc("工作").exists()) { //等待加载出主页
        console.info("等待加载主页");
        delay(1);
    }
