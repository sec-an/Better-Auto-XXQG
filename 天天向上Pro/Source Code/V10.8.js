...
var hamibot={env:{wenzhang:"true",meizhou:"daoxu",zhuanxiang:"daoxu",dingyue:"zhengxu",shipin:"true",meiri:"true",siren:"true",bendi:"true",shuangren:"true",dacuo_num:"2",yinliang:"0",tiaozhan:"true",duizhan_mode:"suiji",guaji:true,is_exit:true,ddtong:false,jisu:"",ocr_maxtime:"",slide_verify:"300",pinglun:"true",yl_on:true,zhanghao:"11111111111:11111111111\n22222222222:22222222222",APP_ENV:"production",USER_ID:"",ROBOT_ID:"",SCRIPT_ID:"",TIMESTAMP:""},plan:{name:"免费",onFreeTrial:false,model:"free"},robotName:"",};
hamibot.postMessage=function(){};hamibot.exit=function(){exit();};
auto.waitFor();//mode = "fast"
var delay_time = 3000;
device.wakeUpIfNeeded();
// 读取自定义配置
const { watchdog, slide_verify, fast_mode, ddtong, is_exit, pinglun, shipin, wenzhang, meiri, meizhou, zhuanxiang, tiaozhan, ocr_plugin,
...
function ocr_test() {
  ...
  try {
    ...
    if (test_time>test_limit) {
      ...
      fError("如偶然变慢，可能为无障碍服务抽风，建议重启hamibot后重试");
      sleep(3000);
      return false
    } else {
      fInfo("OCR功能正常");
...
//   });
  ...
  var meizhou_right = 1;
  if (meizhou != "false" && jifen_list.child(jifen_map["每周"]).child(2).text().match(/\d+/)[0] == "0") { // 每周答题
    toastLog("每周答题开始");
  ...
  if (zhuanxiang != "false" && jifen_list.child(jifen_map["专项"]).child(2).text().match(/\d+/)[0] == "0") { // 专项答题
    toastLog("专项答题开始");
  ...
  var dingyue_right = 1;
  if (dingyue != "false" && jifen_list.child(jifen_map["订阅"]).child(2).text().match(/\d+/)[0] == "0") { // 订阅
    toastLog("订阅开始");
