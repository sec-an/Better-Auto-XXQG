...
var hamibot={env:{fenxiang:"true",wenzhang:"true",meizhou:"daoxu",zhuanxiang:"daoxu",dingyue:"zhengxu",shipin:"true",meiri:"true",siren:"true",bendi:"true",shuangren:"true",dacuo_num:"2",yinliang:"0",tiaozhan:"true",duizhan_mode:"suiji",guaji:true,is_exit:true,watchdog:"",ddtong:false,jisu:"",ocr_maxtime:"",yl_on:true,zhanghao:"11111111111:11111111111\n22222222222:22222222222",APP_ENV:"production",USER_ID:"",ROBOT_ID:"",SCRIPT_ID:"",TIMESTAMP:""},plan:{name:"免费",onFreeTrial:false,model:"free"},robotName:"",};
hamibot.postMessage=function(){};hamibot.exit=function(){exit();};
auto.waitFor();//mode = "fast"
var delay_time = 3000;
device.wakeUpIfNeeded();
// 读取自定义配置
const { watchdog, fast_mode, ddtong, is_exit, fenxiang, shipin, wenzhang, meiri, meizhou, zhuanxiang, tiaozhan, ocr_plugin,
       ocr_maxtime, duizhan_mode, jisu, guaji, siren, dacuo_num, shuangren, bendi, dingyue, pushplus, yl_on, yinliang, zhanghao } = hamibot.env;
...
// 脚本版本号
var last_version = "V10.0";
var engine_version = "V10.1";
var newest_version = "V10.1";
...
// 下载题库
fInfo("检测题库更新");
const update_info = get_tiku_by_http("https://gitee.com/songgedodo/songge_tiku/raw/master/info.json");
...
//   log("文章click: true");
  ...
  while (text_edit.exists()) {
    let pinglun = text_edit.findOne(500);
    fInfo("尝试点击评论框中");
    log(pinglun.click());
    sleep(1500);
    fRefocus();
...
function ocr_test() {
  ...
  try {
    ...
    if (test_time>test_limit) {
      fError("OCR识别过慢(>"+test_limit+"ms)，已跳过多人对战");
      fError("建议重启hamibot后重试");
      return false
    } else {
      fInfo("OCR功能正常");
      img_test.recycle();
...
.item .bar div{height:10px;background-color:#ed4e45;border-radius:5px;}</style>';
  ...
  for (let option of jifen_list.children()) {
    ...
    let score = option.child(2).text().match(/\d+/g)[0];
    let total = option.child(2).text().match(/\d+/g)[1];
...
function refind_jifen() {
  ...
  //log(res);
  let jifen_obj = className("android.widget.ListView").depth(21).rowCount(15).findOne();
  return jifen_obj
}
...
function noverify() {
  var noverify_thread = threads.start(function() {
    ...
    while (true) {
      ...
      var back_x = (x_end-x_start)*random(1,2)/10;
